...
- 通道重叠：FFT的频率分辨率为Δf=fsa/N，即通道之间频率间隔fsep=Δf=fsa/N，而单通道幅度响应为sinc函数，其主瓣宽度为2Δf，这导致通道间存在重叠，通道的重叠现象在加窗之后仍然存在（加窗只是加快旁瓣衰减，主瓣宽度仍然为2Δf）。并且sinc函数通带内幅度变换太大（矩形滤波器相位是线性的），无法直接使用fft或加窗fft来进行通道划分。
