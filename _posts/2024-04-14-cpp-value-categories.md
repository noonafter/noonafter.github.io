...
C++ 中每个表达式都具有一个值类别（value category）。值类别决定了表达式能否取地址、能否作为移动操作的源，以及编译器如何处理临时对象。

从 C++11 起，值类别体系基于两个独立属性构建，形成三个基本类别：`lvalue`、`xvalue`、`prvalue`。
...
两个二元属性理论上有四种组合，但"无身份且不可移动"在 C++ 中没有实际用途，因此只保留三种基本类别：
...
- **rvalue**（右值）= `xvalue` + `prvalue`，即所有可移动的表达式（`m`）,位于右下角
...
关键特征：对象仍存在（有地址），但其资源可被转移走。

### 4、glvalue 与 rvalue

这两个类别是泛化分组，不直接对应具体表达式，用于语言规则的统一描述：

- **glvalue**：涵盖所有有身份的表达式（`lvalue` + `xvalue`），用于描述"可寻址"的场景。
- **rvalue**：涵盖所有可移动的表达式（`xvalue` + `prvalue`），用于描述"可绑定到右值引用"的场景。
...
`T&&` 是右值引用类型，用于绑定右值（`xvalue` 或 `prvalue`）。其主要用途是触发移动构造函数或移动赋值运算符，避免不必要的深拷贝。
...
物理属性（是否有名字）决定值类别，类型属性（是否为 `&&`）决定绑定能力，两者相互独立。
...
- **字符串字面量是左值**，非字符串字面量（`42`、`true`）是 prvalue。
