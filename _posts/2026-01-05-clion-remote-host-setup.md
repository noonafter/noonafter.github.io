...
目标：使用 CLion Remote Host 连接远程 ARM 主机，建立本地编码、远端编译调试的开发模式，跑通 Qt5 的 demo 程序。

前提：远程主机上已经部署 Qt 环境。离线 ARM 主机上 Qt 5.14.2 的依赖获取和源码编译见[离线 ARM Linux 主机上的 Qt 5.14.2 环境搭建](./2026-01-04-offline-arm-linux-qt-setup.md)，本文直接使用其安装结果 `/opt/Qt5.14.2`。
...
配置完成后，目标主机即可通过开发机访问互联网，用于后续安装 `openssh-server`、下载软件包等。Windows主机长时间休眠后，可能会出现共享自动关闭（显示勾选，但没生效）的情况，进入网卡属性中，关闭共享，确认后，再次打开共享，然后再次确认，即可重新生效。
...
### 3.2 配置 CMake Profile

Settings → Build, Execution, Deployment → **CMake**

添加一个 CMake Profile，将 Toolchain 指向上一步创建的远程工具链。CMake 选项中添加 Qt 安装路径：

```
-DCMAKE_PREFIX_PATH=/opt/Qt5.14.2
```

### 3.3 配置 Deployment（文件同步）
...
这告诉 Qt 程序将窗口渲染到 Windows 上运行的 VcXsrv，这一步需要提前完成[第二步](#第-2-步windows-安装-vcxsrv)。
