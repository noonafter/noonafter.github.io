...
本文主线聚焦初始化语法及其语义路径。类型来源和指针、引用分类见[C++ 初始化方式与类型分类](./2024-04-10-cpp-initialization-types.md)；对象生命周期、赋值和异常安全见[C++ 初始化与赋值的底层机制](./2024-04-10-cpp-initialization-assignment.md)。这几篇文章互为补充，不构成必须先读的强依赖。引用的独立规则见[引用初始化](https://en.cppreference.com/w/cpp/language/reference_initialization)。非局部变量还涉及初始化时机问题，见[非局部变量的初始化时机](#非局部变量的初始化时机)。
