...
## 📦 安装
MSYS2的安装非常简单！到[官网](https://www.msys2.org/)下载安装包，双击运行，一路Next就能搞定。安装路径建议保持默认（如`C:\msys64`），避免权限问题。安装完成后，勾选立即运行MSYS2的选项，就能直接进入UCRT64环境终端了。
...
| `mingw64/` | 64位MinGW工具链环境（C运行库为msvcrt，兼容win7及更早版本，适合开发需要兼容旧版Windows的应用程序） |
| `ucrt64/` | UCRT运行时环境（C运行库为ucrt，完整支持C11/C17等最新标准，兼容Win10/11，微软推荐的新开发标准环境） |
| `clang64/` | Clang编译器环境（LLVM党必备） |
