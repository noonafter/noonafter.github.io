...
本文以 Linux 环境为例，梳理 C/C++ 程序从源文件到可执行文件或库的构建过程。这里的“构建”不只指编译器翻译代码，还包括链接产物和构建系统对整个过程的组织。

可以先用三层结构建立整体认识：

```text
构建系统（Make、CMake、Ninja 等）
  -> 组织依赖关系，决定哪些目标需要重新生成

编译工具链（gcc/g++、cc1、as、ld 等）
  -> 预处理、编译、汇编、链接

构建产物（目标文件、静态库、共享库、可执行文件）
```
...
其中，**预处理、编译、汇编和链接**是工具链完成的具体阶段；**Make、CMake 等构建系统**负责描述目标、依赖和构建命令，并协调多个源文件及多个目标之间的生成关系。后文先介绍工具链的实际执行，再说明 Make 和 CMake 如何将这些步骤组织成可重复的构建流程。
...
*   **过程**：包括词法分析、语法分析、语义分析、中间代码生成和优化等一系列复杂操作，编译阶段可进一步分为前端、中端和后端。
*   **阶段输出**：`.s` 文件（汇编代码文件）。

#### 阶段3：汇编（Assemble）

*   **核心任务**：将人类可读的汇编代码（`.s`）逐条翻译成机器可以识别的**二进制指令**，并打包成目标文件格式（生成文件头、符号表、可重定位条目、节头表等）。
*   **阶段输出**：`.o` 文件（目标文件，Object File）。多个相关的目标文件可以被打包成**库（Library）**。

#### 阶段4：链接（Link）

*   **核心任务**：将多个目标文件（`.o`）和所需的库文件拼接在一起，合并成一个可加载执行的文件。
*   **关键步骤**：
    *   **符号解析**：将每个符号（函数名、变量名）的引用和一个符号定义对应起来，进行符号判决，处理强弱符号
    *   **重定位**：将不同目标文件中相同节进行合并，确定每个符号的最终内存地址，并修正代码节和数据节中对这些符号的引用
*   **最终输出**：可执行文件或者动态库文件。值得注意的是，链接分为**静态链接**（库代码被复制到可执行文件中）和**动态链接**（库代码在加载/运行时才被载入内存），在构建可执行文件/动态库的过程中，只使用了静态链接器ld，动态链接器在程序加载和运行阶段才会进行动态链接。静态库不由静态链接器ld生成，而是由静态库制作工具ar生成的。

> **注意**：我们通常调用的 `gcc` 或 `g++` 命令，实际上是一个 **“编译器驱动程序（Compiler Driver）”**。它本身并不执行具体的编译工作，而是根据我们的需求（和命令行参数）去调度后端的各个专业工具完成上述阶段。另外，我们可以用 `gcc` 的选项来精确控制停在某个阶段。
